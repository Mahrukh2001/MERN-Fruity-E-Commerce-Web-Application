  ...
  return (
    <div className='bg-slate-200 p-2 flex gap-4 rounded border border-slate-300'>
        ...
        <div className='flex flex-col gap-1 w-full'>
          ...
          <h3 className='font-semibold text-slate-600 capitalize text-lg md:text-xl mt-1'>{name}</h3>
          <div className='cursor-pointer text-slate-700 hover:text-red-500' onClick={()=>dispatch(deleteCartItem(id))}>
            <AiFillDelete/>
          </div>
          </div>
          <p className='text-slate-500 font-medium'>{category}</p>
          <p className='font-bold text-base'> <span className='text-red-500 px-1'>₨</span>
           <span>{price}</span></p>
          ...
          <div className='flex items-center gap-2 font-bold text-slate-700 px-2'>
            <p>Total</p>
            <p> <span className='text-red-500 px-1'>₨</span>{total}</p>
          </div>
          </div>
        </div>
